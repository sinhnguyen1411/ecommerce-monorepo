...
func (s *Server) CreateOrder(c *gin.Context) {
  ...
  result, err := tx.Exec(`
    INSERT INTO orders (order_number, customer_name, email, phone, address, note, delivery_time, subtotal, shipping_fee, discount_total, total, payment_method, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
  `, orderNumber, input.CustomerName, input.Email, input.Phone, input.Address, input.Note, input.DeliveryTime, subtotal, shippingFee, discountTotal, total, input.PaymentMethod)
  if err != nil {
...
func (s *Server) UploadPaymentProof(c *gin.Context) {
  ...
  if _, err := s.DB.Exec("UPDATE orders SET payment_proof_url = ? WHERE id = ?", url, orderID); err != nil {
