...
Haravan.onCartShippingRatesUpdate = function (rates, shipping_address) {
  ...
  alert(
    "There are " +
      ...
      readable_address +
      ", starting at " +
      Haravan.formatMoney(rates[0].price) +
      "."
  );
};
...
Haravan.formatMoney = function (cents, format) {
  ...
  if (typeof cents == "string")
    cents = cents.replace(Haravan.format.thousands, "");
  var value = "";
  var patt = /\{\{\s*(\w+)\s*\}\}/;
...
Haravan.resizeImage = function (image, size) {
  try {
    if (size == "original") {
      ...
      return (
        matches[1].replace(/http:/g, "").replace(/https:/g, "") +
        "_" +
        size +
        "." +
        matches[2]
...
Haravan.getCartShippingRatesForDestination = function (
  shipping_address,
  callback
) {
  var params = {
